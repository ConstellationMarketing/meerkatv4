 ...
 */
async function retryFailed(batchId, articles) {
  const { data: job } = await supabase
    .from(BATCH_TABLE)
    .select('*')
    .eq('batch_id', batchId)
  ...
  // Get the failed keywords
  const failedKeywords = new Set(job.errors.map(e => e.keyword));

  // Filter articles to only failed ones
  const retryArticles = articles.filter(a => failedKeywords.has(a.keyword));
  ...
  await getSupabase().from(BATCH_TABLE).update({
    ...
    failed_count: 0,
    errors: [],
    total_articles: retryArticles.length,
    completed_count: 0,
  }).eq('batch_id', batchId);

  console.log(`[Batch] Retrying ${retryArticles.length} failed articles from batch "${batchId}"`);
...
module.exports = { startBatch, cancelBatch, retryFailed, getBatchStatus };
