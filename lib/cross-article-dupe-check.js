...
/**
 * Fetch distinctive sentences from recent same-client articles to inject into
 * the section-writer prompt as "do not reuse" guidance. Prevention companion
 * to checkCrossArticleDuplicates — instead of warning post-generation, we
 * tell the model up-front which phrases have already shipped for this client.
 *
 * Gated by CROSS_ARTICLE_DEDUP_PREVENT env var (separate from the detection
 * gate so the team can toggle each independently).
 *
 * @param {string} clientName
 * @param {string} [excludeArticleId]   - skip this article_id
 * @param {object} [opts]
 * @param {number} [opts.priorLimit=3]  - articles to read
 * @param {number} [opts.maxSentences=30] - cap on returned sentences
 * @param {number} [opts.minWords=8]    - min content-word length
 * @returns {Promise<string[]>}         - array of sentences (already deduped)
 */
async function getPriorClientPhrases(clientName, excludeArticleId, opts = {}) {
  if (!process.env.CROSS_ARTICLE_DEDUP_PREVENT) return [];
  if (!clientName) return [];

  const priorLimit = opts.priorLimit ?? 3;
  const maxSentences = opts.maxSentences ?? 30;
  const minWords = opts.minWords ?? 8;
  ...
  try {
    ...
    let q = getClient()
      .from(table)
      .select('article_id, received_article, "cleaned content"')
      .eq('client_name', clientName)
      .order('created_at', { ascending: false })
      .limit(priorLimit + 1);
    if (excludeArticleId) q = q.neq('article_id', excludeArticleId);
    const { data, error } = await q;
    if (error) {
      console.warn('[DedupPrevent] fetch failed:', error.message);
      return [];
    }
    priorContents = (data || [])
      .slice(0, priorLimit)
      .map(row => row.received_article?.content || row['cleaned content'] || '')
      .filter(c => c);
  } catch (e) {
    console.warn('[DedupPrevent] error fetching prior articles:', e.message);
    return [];
  }

  if (!priorContents.length) return [];

  // Extract all qualifying sentences, dedupe (case-insensitive trim), cap.
  const seen = new Set();
  const out = [];
  for (const html of priorContents) {
    for (const s of extractSentences(html)) {
      if (tokenize(s).length < minWords) continue;
