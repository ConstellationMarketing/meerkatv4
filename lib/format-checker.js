...
function checkAndFixFormat(html, options = {}) {
  ...
  // Intro is content between H1 and first H2
  ...
  if (introMatch) {
    ...
    if (introWords < 200) {
      warnings.push(`Introduction is only ${introWords} words (minimum 200)`);
    }
  }
  ...
  if (h1Count > 1) {
    warnings.push(`Found ${h1Count} H1 tags (must be exactly 1)`);
  }
  if (h1Count === 0) {
    warnings.push('No H1 tag found');
  }

  // ─── 9. Bold entire paragraphs ─────────────────────────────────────────
  const boldParas = fixed.match(/<p>\s*<strong>[^<]{100,}<\/strong>\s*<\/p>/g);
  if (boldParas) {
    warnings.push(`Found ${boldParas.length} paragraph(s) that are entirely bold (100+ chars)`);
  }

  // ─── 10. Links in headings ─────────────────────────────────────────────
  const linkedHeadings = fixed.match(/<h[1-6]>[^<]*<a [^>]*>[^<]*<\/a>[^<]*<\/h[1-6]>/g);
  if (linkedHeadings) {
    warnings.push(`Found ${linkedHeadings.length} heading(s) containing links — links should be in body text only`);
  }

  // ─── 11. H2 immediately followed by H3 with no intro paragraph ─────────
  const h2h3Pattern = /<\/h2>\s*<h3>/gi;
  const h2h3Matches = fixed.match(h2h3Pattern);
  if (h2h3Matches) {
    warnings.push(`Found ${h2h3Matches.length} section(s) where H2 is immediately followed by H3 with no intro paragraph`);
  }

  // ─── 12. CTA section too short (practice pages) ───────────────────────
  if (options.template && (options.template.toLowerCase().includes('practice'))) {
    // Find the last H2 section before FAQ
    const h2Parts = fixed.split(/<h2>/i);
    // Look for the CTA section (usually second-to-last, before FAQ)
    for (let i = h2Parts.length - 1; i >= 1; i--) {
      ...
      // This is likely the CTA section
      const ctaText = part.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
      const ctaWords = ctaText.split(/\s+/).length;
      if (ctaWords < 100) {
        warnings.push(`CTA section "${heading.trim()}" is only ${ctaWords} words (minimum 100 for practice pages)`);
      }
      break;
    }
  }

  // ─── 13. Missing required sections on practice pages ──────────────────
  if (options.template && options.template.toLowerCase().includes('practice')) {
    const allH2s = (fixed.match(/<h2>([^<]*)<\/h2>/gi) || []).map(h => h.replace(/<\/?h2>/gi, '').toLowerCase());
    const hasWhyChoose = allH2s.some(h => h.includes('why choose'));
    if (!hasWhyChoose) {
      warnings.push('Practice page is missing a "Why Choose Us" section');
    }
  }

  // ─── 14. Intro paragraph count ────────────────────────────────────────
  if (introMatch) {
    const introContent = introMatch[1];
    const introParagraphs = (introContent.match(/<p>/gi) || []).length;
    if (introParagraphs < 3) {
      warnings.push(`Introduction has only ${introParagraphs} paragraph(s) (minimum 3 recommended)`);
    }
  }

  // ─── 15. FAQ answer length check ──────────────────────────────────────
  const faqSection = fixed.match(/<h2>[^<]*(FAQ|Frequently Asked)[^<]*<\/h2>([\s\S]*?)(?=<h2>|$)/i);
  if (faqSection) {
    const faqAnswers = faqSection[2].match(/<p>([\s\S]*?)<\/p>/gi) || [];
    faqAnswers.forEach((answer, idx) => {
      const answerText = answer.replace(/<[^>]+>/g, '').trim();
      const answerWords = answerText.split(/\s+/).length;
      if (answerWords > 60) {
        warnings.push(`FAQ answer ${idx + 1} is ${answerWords} words (recommended max ~60 words / 2 sentences)`);
      }
    });
  }

  // ─── 16. Client info quality check ─────────────────────────────────────
