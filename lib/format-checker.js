...
function checkAndFixFormat(html, options = {}) {
  ...
  if (isPracticePage) {
    const allHeadings = (fixed.match(/<h2>[^<]*<\/h2>/gi) || []).map(h => h.replace(/<[^>]+>/g, '').trim().toLowerCase());

    const hasHowWeCanHelp = allHeadings.some(h => /how\s+we\s+can\s+help|how\s+\w+\s+can\s+help|how\s+\w+\s+helps/i.test(h));
    const hasWhyChooseUs = allHeadings.some(h => /why\s+choose/i.test(h));

    if (!hasHowWeCanHelp) {
      warnings.push('REQUIRED: Missing "How We Can Help" section — required on practice pages');
    }
    if (!hasWhyChooseUs) {
      warnings.push('REQUIRED: Missing "Why Choose Us" section — required on practice pages');
    }
  }
