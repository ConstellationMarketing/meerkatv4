...
function insertLinks(htmlContent, references) {
  ...
  // Track URLs already present in the HTML to avoid duplicates
  const existingUrls = new Set();
  const existingUrlMatches = htmlContent.match(/href="([^"]*)"/gi) || [];
  existingUrlMatches.forEach(m => {
    const url = m.match(/href="([^"]*)"/i);
    if (url) existingUrls.add(url[1].toLowerCase().replace(/\/+$/, ''));
  });

  references.forEach((ref, index) => {
    if (!ref.term || !ref.url) return;

    // Skip if this URL is already linked in the article
    const normalizedUrl = ref.url.toLowerCase().replace(/\/+$/, '');
