...
function parseTranslationResponse(raw) {
  ...
  try {
    ...
    throw new Error(`Translator returned invalid JSON: ${err.message}`);
  }
...
async function runTranslation(articleId, language) {
  ...
  const sourceContent = article['cleaned content'];
  if (!sourceContent) throw new Error(`No cleaned content for article: ${articleId}`);

  const sourceTitle = article.title_tag || article.received_article?.title || '';
  const sourceMeta = article.meta_description || article.received_article?.meta || '';
