...
export const handler = async (event: any) => {
  ...
  try {
    if (method === "GET") {
      ...
      console.log(`[article-access API] GET: Fetching accesses for article ${articleId}`);
      ...
      console.log(`[article-access API] GET: Found ${data?.length || 0} access records for article ${articleId}`);
    ...
    if (method === "POST") {
      ...
      console.log(`[article-access API] POST: Adding access for ${normalizedEmail} to article ${articleId}`);
      ...
      // Use upsert to handle duplicate emails (update if exists, insert if not)
      const { data, error } = await supabase
        .from("article_access")
        .upsert(
          {
